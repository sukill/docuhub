...
class GitPlumbing:
    ...
    def hash_object(self, repo_path, content):
        output = self._run_git(repo_path, ["hash-object", "-w", "--stdin"], input=content)
    ...
    def commit_tree(self, repo_path, tree_hash, parent_hash=None, message="Commit", author_name="Admin", author_email="admin@example.com"):
        env = os.environ.copy()
    ...
    def list_files(self, repo_path, ref, path):
        ...
        for line in lines:
            ...
            mode, type_, hash_, size_str = parts[0], parts[1], parts[2], parts[3]
            name = line.split('\t')[1]
